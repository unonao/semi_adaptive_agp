...
def load_graphsage_data(dataset_path, dataset_str, normalize=True):
  ...
  num_edges_self = full_adj.nonzero()[0].shape[0]
  # for homophily
  A = full_adj.tocoo()
  ...
  stats_str = f"""
    nodes:{num_data}, edges:{num_edges_self}, edges w/ self-loop:{num_edges_self}, features:{num_features}, classes:{num_classes}, train:{len(train_data)}, val:{len(val_data)}, test:{len(test_data)}
    """
...
def Amazon2M(datastr, dataset):
  _, train_adj, full_adj, feats, train_feats, _, labels, idx_train, idx_val, idx_test = load_graphsage_data(datastr, dataset, normalize=True)
...
if __name__ == "__main__":
  ...
  print(args.datastr)
  Amazon2M(args.datastr,"Amazon2M")
