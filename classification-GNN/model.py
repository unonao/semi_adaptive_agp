...
class SaGnnAGP(nn.Module):
    def __init__(self, nagp, nfeat, nlayers,nhidden, nclass, dropout, bias):
        super(SaGnnAGP, self).__init__()
        self.linear = nn.Linear(nagp, 1, bias=False)
        self.fcs = nn.ModuleList()
        self.fcs.append(Dense(nfeat, nhidden, bias))
        for _ in range(nlayers-2):
            self.fcs.append(Dense(nhidden, nhidden, bias))
...
class SaMLP(torch.nn.Module):
    def __init__(self,nagp, in_channels, hidden_channels, out_channels, num_layers,dropout):
