...
double Agp::agp_operation(string dataset_el,string dataset_pl,string dataset,string agp_alg,uint mm,uint nn,int LL,double rmaxx,double alphaa,double tt,Eigen::Map<Eigen::MatrixXd> &feat)
{

    int NUMTHREAD=thread::hardware_concurrency(); //Number of threads
    ...
    cout<<"The propagation time: "<<timeCost<<" s"<<endl; //経過実時間
    cout<<"The clock time : "<<total_t<<" s"<<endl; // プロセスが消費するCPU時間（CLOCK_PROCESS_CPUTIME_IDと同じ）
